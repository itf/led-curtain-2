...
def __rotate(pattern):  return defaultArgsP(rotateAngle=0, rotatex0=0.5, rotatey0=0.5)(arg('yTranslate="sin(rotateAngle)*(x-rotatex0)+cos(rotateAngle)*(y-rotatey0)-y-rotatey0"; xTranslate="cos(rotateAngle)*(x-rotatex0)-sin(rotateAngle)*(y-rotatey0)-x-rotatex0"')(translate(pattern)))
@Function.function('__wave')
def __wave(pattern):  return defaultArgsP(waveSlantY=0.5, waveYTransvAmp=0.2, waveYTransvPhase=0, waveYTransvFreq=0.5, waveYLongAmp=0, waveYLongFreq=1, waveYLongPhase=0, waveYPos=0, waveSlantX=-0.5, waveXTransvAmp=0.2, waveXTransvPhase=0, waveXTransvFreq=0.5, waveXLongAmp=0, waveXLongFreq=1, waveXLongPhase=0, waveXPos=0)(arg('yTranslate="x*waveSlantY+waveYTransvAmp*sin((x*waveYTransvFreq+waveYTransvPhase)*2*pi) +waveYLongAmp*sin((y*waveYLongFreq+waveYLongPhase)*2*pi) + waveYPos";xTranslate="y*waveSlantX+waveXTransvAmp*sin((y*waveXTransvFreq+waveXTransvPhase)*2*pi) +waveXLongAmp*sin((x*waveXLongFreq+waveXLongPhase)*2*pi) + waveXPos"')(translate(pattern)))
